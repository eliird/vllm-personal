...
T0=$(date +%s.%N)
echo "launch_command: $VLLM_BIN serve $MODEL --port $PORT --max-model-len $MAX_MODEL_LEN --gpu-memory-utilization $GPU_MEM_UTIL $EXTRA_ARGS" | tee -a "$VLOG"
setsid env PYTHONUNBUFFERED=1 "$VLLM_BIN" serve "$MODEL" --port "$PORT" \
