...
Works on logs produced by snapshot/scripts/p1_baseline.sh (timestamp-prefixed)
and on older logs, using either:
  - a `=== harness_summary ===` block inside the log, or
  - the fallback file snapshot/logs/p1_wall_clock_summary.txt.

Usage:
    python snapshot/scripts/p1_parse_breakdown.py snapshot/logs/p1_*_vllm.log
    python snapshot/scripts/p1_parse_breakdown.py \
        --plot snapshot/plots/startup_timeline.png snapshot/logs/p1_*_vllm.log
"""
...
def main() -> None:
    ...
    ap.add_argument("--summary",
                    default=os.path.normpath(os.path.join(
                        ...
                        "..", "logs", "p1_wall_clock_summary.txt")),
    ...
    if args.markdown:
        with open(args.markdown, "w") as f:
            f.write("# Startup time breakdown\n\nAuto-generated by "
                    "`snapshot/scripts/p1_parse_breakdown.py`.\n\n")
